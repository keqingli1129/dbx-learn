...
One project. Scaffolded by `databricks bundle init default-python` into a temporary directory and
moved to the repository root, because `bundle init` has no in-place mode (research R11). No uv:
the template's `artifacts: uv build --wheel` block was removed, since pipelines install this
project with `--editable ${workspace.file_path}` and no wheel is needed.

```text
dbx-learn/                          # the bundle IS the repository
├── databricks.yml                  # name, uuid, dev + prod targets. No artifacts block.
