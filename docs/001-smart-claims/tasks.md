...
- [ ] T063 [US5] Create `resources/transform.pipeline.yml` with default catalog `smart_claims_dev`, default schema `silver`, serverless, `root_path: ../src/smart_claims_etl` and two glob includes (`transformations/silver/**`, `transformations/gold/**`), gold datasets written by fully-qualified name, and `environment.dependencies: ["--editable ${workspace.file_path}"]` so the pipeline can import `smart_claims.lib.cleaning` (research R11). Declare no `torch` dependency here — this pipeline does not need it
