...
- [X] T005 Add dependencies to `pyproject.toml` (which already has `[build-system]` from the scaffold — verify it packages `src/smart_claims`): runtime `fastapi`, `uvicorn`, `python-multipart`, `databricks-sdk`; dev group `pytest`, `pytest-mock`. Do NOT add `pyspark` — it conflicts with `databricks-connect`. No uv: dependencies are installed with `pip install -e '.[dev]'` into a plain virtualenv. Note the scaffold's own warning that pipeline dependencies belong in each pipeline's `environment` block, not here (research R11)
- [X] T006 [P] Configure pytest in `pyproject.toml`: `testpaths = ["tests"]`, and register the marker `workspace` for tests requiring a live profile, with `addopts = "-m 'not workspace'"` so `pytest` runs offline by default
- [X] T007 [P] Create `tests/__init__.py`, `tests/unit/__init__.py`, `tests/integration/__init__.py`, and `tests/conftest.py` defining a `--profile` option that skips `workspace`-marked tests when absent Additionally, assert at session start that `sys.path` carries no foreign site-packages tree — this machine exports a ROS `PYTHONPATH` that leaks into the venv (research R10) — and fail with an actionable message telling the user to run `env -u PYTHONPATH pytest`. The guard is a backstop only: pytest loads entry-point plugins before conftest, so a foreign tree that registers one crashes startup first — add `run-tests.sh`, which execs pytest with `PYTHONPATH` cleared, and use it everywhere.

**Checkpoint**: `databricks bundle validate -t dev --profile DEFAULT` is clean and `pytest` exits successfully with zero tests collected.
...
- [X] T009 [P] Write `tests/unit/test_payload.py` asserting base64-encoded JSON decodes to a dict with keys `chassis_no`, `speed`, `latitude`, `longitude`, `event_timestamp`; that malformed base64 and malformed JSON each return a sentinel rather than raising (a single bad payload must not fail ingestion — spec edge case); and that all values arrive as strings
- [X] T010 [P] Write `tests/unit/test_cleaning.py` covering: `"Ada Lovelace"` → `("Ada", "Lovelace")`; single-token and triple-token names; empty and null names; address whitespace and casing normalisation; and date coercion for all four formats present in the data (`MM/dd/yyyy`, `yyyy-MM-dd`, `dd-MM-yyyy`, `dd/MM/yyyy`) plus `yyyy-MM-dd HH:mm:ss`, with unparseable input returning `None` not raising
- [X] T011 [P] Write `tests/unit/test_decision.py` covering the three-valued fold from contracts/rules-engine.md: all `pass` → `release_funds`; any `fail` → `requires_investigation`; **any `indeterminate` → `requires_investigation`**; empty rule set; and that `indeterminate` is never collapsed to `pass` or `fail`

### Implementation

- [X] T012 [P] Implement `src/smart_claims/lib/severity.py` — `SEVERITY_DOMAIN` (immutable, ordered least-to-most-severe), `rank()`, `normalize()` (case/whitespace-insensitive, raises `UnknownSeverityError` on unknown or non-string rather than defaulting), `MODEL_LABELS` = `("okay", "minor", "major")`, `from_model_label()` (case-insensitive), and `MODEL_UNREACHABLE` = `("Total Loss",)`. No Spark import. The API is pinned by `tests/unit/test_severity.py` (T008)
- [X] T013 [P] Implement `src/smart_claims/lib/payload.py` — `decode_telematics(data) -> dict[str, str] | None`. Returns `None` for every decode failure (bad base64, bad padding, non-UTF-8, malformed JSON, JSON that is not an object, non-string input) and MUST NOT raise on any input. Coerces every JSON scalar to `str`, preserves extra keys, and returns a partial mapping when a documented key is missing rather than treating that as a decode failure. No Spark import. The API and every failure path are pinned by `tests/unit/test_payload.py` (T009)
- [X] T014 [P] Implement `src/smart_claims/lib/cleaning.py` — `split_name` (first token is the given name, the remainder is the surname; `(None, None)` for empty/missing/non-string), `normalize_address` (strip, collapse whitespace, uppercase per postal convention; `None` for empty/non-string), `DATE_FORMATS` (a mapping of the five source formats, each pairing its `.python` strptime pattern with its `.spark` pattern so the two never drift), `parse_date(value, fmt)` and `parse_timestamp(value, fmt)` — **`fmt` is a required argument, never inferred**, and both return `None` on anything unparseable rather than raising. No Spark import. The API is pinned by `tests/unit/test_cleaning.py` (T010)
- [X] T015 [P] Implement `src/smart_claims/lib/decision.py` — `CheckResult` (`PASS`/`FAIL`/`INDETERMINATE`, values `pass`/`fail`/`indeterminate`, and NOT bool-coercible so `if result:` cannot silently misread one), `Outcome` (`release_funds`/`requires_investigation`), `from_sql_bool` (`True`→PASS, `False`→FAIL, `None`→INDETERMINATE, `TypeError` on anything else so `0` or `""` is never coerced), and `overall_outcome(results)` returning `release_funds` only when at least one result was given and all are PASS. No Spark import. Pinned by `tests/unit/test_decision.py` (T011)
- [X] T016 Create `resources/catalog.yml` declaring catalog `smart_claims_dev` and schemas `source`, `landing`, `bronze`, `silver`, `gold`, plus managed volumes `landing.telematics_raw`, `landing.claims`, `landing.training_images` (depends on T001)
- [X] T017 Create `src/smart_claims/lib/config.py` — the single place that turns pipeline/job configuration into fully-qualified names. Reads `catalog`, the five **resolved** schema names, `speed_threshold` and the volume roots from configuration, and exposes helpers such as `table("bronze", "telematics")` and `volume_path("telematics_raw")`. No hardcoded catalog or schema name anywhere — `mode: development` renames schemas to `dev_<user>_<name>` (research R13), so a literal would be wrong at the `dev` target. No Spark import. Covered by `tests/unit/test_config.py`, added unplanned because R13 made this module the single point where a wrong name would silently redirect every read.
- [X] T018 Run `./bootstrap.sh dev DEFAULT` to create the catalog (the REST API refuses it on a Default Storage account — research R14), then `databricks bundle deploy -t dev --profile DEFAULT`, and confirm the five schemas and three volumes exist via `databricks schemas list` and `databricks volumes list`
...
- [X] T019 [US1] Implement `src/smart_claims/setup/seed_source_data.py` generating ~1000 customers, ~1200 policies, ~1300 claims per the schemas in data-model.md, writing to `source.customer`, `source.policy`, `source.claim` with `delta.enableChangeDataFeed = true` AND `delta.enableRowTracking = true` on all three (row tracking is required for gold MVs to refresh incrementally rather than silently full-recompute — research R6, FR-023) Written pure-core with Spark imported lazily inside `main()`, so the generators are verifiable offline against `lib/` before any deploy — the date formats come from `lib.cleaning.DATE_FORMATS` and the severities from `lib.severity.SEVERITY_DOMAIN`, so the seed data cannot disagree with the code that consumes it. Uses stdlib `random`, not Faker (absent from serverless, research R1).
- [X] T020 [US1] Extend `seed_source_data.py` to inject the **deliberate defects** the later phases prove they catch: some `claim.claim_no` NULL; some `claim.incident_hour` outside 0–23; some `policy.premium` negative; some `claim.total_claim_amount` zero or negative; `customer.name` always a combined `"First Last"`; `customer.address` with inconsistent whitespace and casing. Dates written as text in four distinct formats — `claim_date` as `yyyy-MM-dd HH:mm:ss`, `incident_date` as `MM/dd/yyyy`, `driver_license_issue_date` as `dd/MM/yyyy`, `policy.pol_issue_date` as `dd-MM-yyyy`, `pol_eff_date`/`pol_expiry_date` as `yyyy-MM-dd` Defects are planted on DISJOINT index sets and the injector returns a manifest, so `claims_silver_should_drop` is an exact figure a later task can assert against rather than 'some rows'. Defect VALUES are cycled, not randomly chosen, so every variant (hour below 0 and above 23; amount zero and negative) is guaranteed present even at small row counts. Covered by `tests/unit/test_seed_defects.py`, added unplanned.
- [X] T021 [US1] Ensure referential consistency in `seed_source_data.py`: every `claim.policy_no` resolves to a policy and every `policy.customer_id` to a customer, EXCEPT a small deliberate set of orphan claims to exercise the missing-policy edge case. Assign `policy.chassis_no` values that the telematics producer will later emit for Assignment is SKEWED rather than round-robin (`i % len(...)`): that guaranteed integrity but produced a lattice where every policy had exactly 1-2 claims, so joins and aggregations would only ever be exercised at cardinality 1-2. Now ~460 policies have no claims, some have up to 9, and ~150 customers hold none — giving the gold layer real zero-match and many-match cases. Orphans are drawn only from claims that SURVIVE the silver expectations, so the edge case actually reaches triage. The manifest also reports `chassis_without_telematics`, which T027 must skip.
...
- [ ] T029 [US2] Implement `src/smart_claims_etl/transformations/bronze/telematics.py` — a streaming table `bronze.telematics` reading with Auto Loader (`cloudFiles`, format `json`) from the landing volume, then decoding: base64 → `cast(string)` → `from_json` against an all-string map schema → projection into `stream_metadata STRUCT<partition_key, sequence_number, arrival_ts>` plus typed `chassis_no`, `speed`, `latitude`, `longitude`, `event_timestamp`, plus `_ingested_at`. Decoding calls `lib.payload` via a UDF — do not reimplement the decode inline The UDF wrapping `lib.payload.decode_telematics` must map its `None` sentinel to a null struct so the row survives to silver and is dropped there by the `chassis_no IS NOT NULL` expectation with a counted violation — not filtered out silently in bronze, which would lose the count.
- [ ] T030 [US2] Create `resources/ingest.pipeline.yml` declaring the ingest pipeline with default catalog `smart_claims_dev`, default schema `bronze`, serverless, `continuous: false`, and `root_path: ../src/smart_claims_etl` and `libraries.glob.include: ../src/smart_claims_etl/transformations/bronze/**` so only bronze dataset files are evaluated, plus configuration keys `catalog`, `landing_volume_root`, `schema_evolution_mode`, `clean_source_retention`, `archive_path` (contracts/pipeline-datasets.md). Do NOT hardcode these in pipeline code. Declare `environment.dependencies: ["../dist/*.whl"]` so the pipeline can `import smart_claims.lib` — the `--editable` form does NOT work (research R15) **Unresolved from the scaffold**: `databricks.yml` declares a single `schema` variable (assigned `dev`/`prod`), which does not fit this design — the medallion schemas are fixed (`bronze`, `silver`, `gold`) and differ per pipeline, not per environment. Decide here whether to set the pipeline's schema literally and drop the variable, or repurpose it. Pass the resolved schema names in via `configuration:` using `${resources.schemas.<key>.name}` — never a literal like `bronze`, which does not exist at the dev target (research R13).
...
- [ ] T055 [US5] Implement `silver.claim` in `src/smart_claims_etl/transformations/silver/claim.py` with `@dp.expect_all_or_drop` carrying exactly three constraints — `valid_claim_number: claim_no IS NOT NULL`, `valid_incident_hour: incident_hour BETWEEN 0 AND 23` and `positive_claim_amount: total_claim_amount > 0` (FR-015) — coercing all four date columns using their respective formats via `lib.cleaning.parse_date`, and dropping `_rescued_data` Take the date pattern from `lib.cleaning.DATE_FORMATS[...].spark` rather than writing the pattern string inline — the same table supplies the Python pattern the unit tests assert against, which is what keeps the two from drifting.
- [ ] T056 [P] [US5] Implement `silver.policy` in `src/smart_claims_etl/transformations/silver/policy.py` with `expect_all_or_drop` on `valid_policy_number: policy_no IS NOT NULL`, `premium → abs(premium)` (FR-019), and the three date columns coerced with their distinct formats Take the date pattern from `lib.cleaning.DATE_FORMATS[...].spark` rather than writing the pattern string inline — the same table supplies the Python pattern the unit tests assert against, which is what keeps the two from drifting.
- [ ] T057 [P] [US5] Implement `silver.customer` in `src/smart_claims_etl/transformations/silver/customer.py` with `expect_all_or_drop` on `valid_customer_id: customer_id IS NOT NULL`, `name` split into `first_name`/`last_name` via `lib.cleaning.split_name`, `address` normalised via `lib.cleaning.normalize_address`, and `date_of_birth` coerced Take the date pattern from `lib.cleaning.DATE_FORMATS[...].spark` rather than writing the pattern string inline — the same table supplies the Python pattern the unit tests assert against, which is what keeps the two from drifting.
- [ ] T058 [P] [US5] Implement `silver.telematics` in `src/smart_claims_etl/transformations/silver/telematics.py` with `expect_all_or_drop` on `chassis_no IS NOT NULL` and `speed >= 0`, casting speed/latitude/longitude to DOUBLE and `event_timestamp` to TIMESTAMP Take the date pattern from `lib.cleaning.DATE_FORMATS[...].spark` rather than writing the pattern string inline — the same table supplies the Python pattern the unit tests assert against, which is what keeps the two from drifting.
...
- [ ] T063 [US5] Create `resources/transform.pipeline.yml` with default catalog `smart_claims_dev`, default schema `silver`, serverless, `root_path: ../src/smart_claims_etl` and two glob includes (`transformations/silver/**`, `transformations/gold/**`), gold datasets written by fully-qualified name, and `environment.dependencies: ["../dist/*.whl"]` so the pipeline can import `smart_claims.lib.cleaning` — the `--editable` form does NOT work (research R15). Declare no `torch` dependency here — this pipeline does not need it **Unresolved from the scaffold**: `databricks.yml` declares a single `schema` variable (assigned `dev`/`prod`), which does not fit this design — the medallion schemas are fixed (`bronze`, `silver`, `gold`) and differ per pipeline, not per environment. Decide here whether to set the pipeline's schema literally and drop the variable, or repurpose it. Pass the resolved schema names in via `configuration:` using `${resources.schemas.<key>.name}` — never a literal like `bronze`, which does not exist at the dev target (research R13).
- [ ] T064 [US5] Deploy and run the transform pipeline; verify SC-006 by inspecting the expectations panel — non-zero drops for `valid_claim_number` and `valid_incident_hour` are **expected**, because T020 seeded those defects. Zero drops means the expectations are not firing, not that the data is clean Compare the drop count against the manifest the seed job printed: silver must drop EXACTLY `claims_silver_should_drop` claims — more means an expectation is over-matching, fewer means one is not firing.
...
- [ ] T076 [US6] Implement `src/smart_claims/ml/batch_score.py` producing `gold.claim_images_predicted` — load the model with `mlflow.pyfunc.spark_udf` at alias `@prod` and score `silver.claim_images`, writing `predicted_severity` and `prediction_confidence` alongside `claim_no` **Total Loss is unconfirmable by the model** — it has only three classes (`okay`/`minor`/`major`), so `Total Loss` is unreachable from any prediction (asserted in `tests/unit/test_severity.py`). Decide here whether a claim reporting `Total Loss` yields `indeterminate` for the severity check (the model cannot confirm or refute it, so a human reviews) or `fail`. `indeterminate` is the principled reading under the three-valued contract; `fail` would report a mismatch the model never actually assessed.
- [ ] T077 [US6] Create `resources/ml.job.yml` declaring a serverless job with tasks `resize_images` → `train_classifier` → `batch_score`, its environment declaring `torch`, `torchvision` and a pinned `mlflow>=3` (all absent or outdated on the base image — research R1). Keep training a **separate task** from scoring so scoring can re-run without retraining **Total Loss is unconfirmable by the model** — it has only three classes (`okay`/`minor`/`major`), so `Total Loss` is unreachable from any prediction (asserted in `tests/unit/test_severity.py`). Decide here whether a claim reporting `Total Loss` yields `indeterminate` for the severity check (the model cannot confirm or refute it, so a human reviews) or `fail`. `indeterminate` is the principled reading under the three-valued contract; `fail` would report a mismatch the model never actually assessed.
...
- [ ] T096 [US7] Implement `src/smart_claims/app/main.py` — FastAPI app with startup-time repository selection, and `GET /health` returning `{"status","data_source"}` where `data_source` reports which repository resolved, making S2's outcome observable at runtime The wheel declares no runtime dependencies; the app's web stack lives in the `app` extra, so this must install `smart_claims[app]`, not bare `smart_claims`.
...
- [ ] T102 [US7] Create `resources/app.yml` declaring the app with its SQL warehouse resource, plus the database resource if S2 succeeded and the serving endpoint if S1 succeeded The wheel declares no runtime dependencies; the app's web stack lives in the `app` extra, so this must install `smart_claims[app]`, not bare `smart_claims`.
...
- [ ] T108 Verify SC-017 — deploy the bundle to an empty state and confirm a working system with no undocumented manual steps; correct `quickstart.md` wherever reality diverged from it Note that `bundle destroy` does NOT remove the catalog, since `bootstrap.sh` creates it outside the bundle (research R14) — verify the schemas and volumes are gone, then remove the catalog with `databricks catalogs delete ${var.catalog} --force`.
