...
- [ ] T009 [P] Write `tests/unit/test_payload.py` asserting base64-encoded JSON decodes to a dict with keys `chassis_no`, `speed`, `latitude`, `longitude`, `event_timestamp`; that malformed base64 and malformed JSON each return a sentinel rather than raising (a single bad payload must not fail ingestion — spec edge case); and that all values arrive as strings Additionally, assert at session start that `sys.path` carries no foreign site-packages tree — this machine exports a ROS `PYTHONPATH` that leaks into the venv (research R10) — and fail with an actionable message telling the user to run `env -u PYTHONPATH pytest`.
