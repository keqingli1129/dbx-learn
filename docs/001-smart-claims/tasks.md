...
- [ ] T014 [P] Implement `src/smart_claims/lib/cleaning.py` — `split_name` (first token is the given name, the remainder is the surname; `(None, None)` for empty/missing/non-string), `normalize_address` (strip, collapse whitespace, uppercase per postal convention; `None` for empty/non-string), `DATE_FORMATS` (a mapping of the five source formats, each pairing its `.python` strptime pattern with its `.spark` pattern so the two never drift), `parse_date(value, fmt)` and `parse_timestamp(value, fmt)` — **`fmt` is a required argument, never inferred**, and both return `None` on anything unparseable rather than raising. No Spark import. The API is pinned by `tests/unit/test_cleaning.py` (T010)
- [ ] T015 [P] Implement `src/smart_claims/lib/decision.py` — `CheckResult` (`PASS`/`FAIL`/`INDETERMINATE`, values `pass`/`fail`/`indeterminate`, and NOT bool-coercible so `if result:` cannot silently misread one), `Outcome` (`release_funds`/`requires_investigation`), `from_sql_bool` (`True`→PASS, `False`→FAIL, `None`→INDETERMINATE, `TypeError` on anything else so `0` or `""` is never coerced), and `overall_outcome(results)` returning `release_funds` only when at least one result was given and all are PASS. No Spark import. Pinned by `tests/unit/test_decision.py` (T011)
